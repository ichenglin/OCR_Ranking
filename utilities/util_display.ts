import { APIEmbedField, ActionRowBuilder, AnyComponentBuilder, AttachmentBuilder, BaseMessageOptions, ChatInputCommandInteraction, EmbedBuilder, Message, PermissionFlagsBits, TextBasedChannel, TextChannel } from "discord.js";
...
export default class VerificationDisplay {
    ...
    public static async embed_editreply(interaction: ChatInputCommandInteraction, embed_group: BaseMessageOptions[], embed_messages_old: Message<boolean>[] = []): Promise<Message<boolean>[]> {
        ...
        for (const embed_children of embed_group.slice(1)) embed_messages.push(await (interaction.channel as TextChannel).send(embed_children));
